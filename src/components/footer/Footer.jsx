...
export default function Footer() {
    ...
    const footerStyle = {
        backgroundColor: footer.bg_type === 'color' ? footer.bg_color : 'transparent',
        backgroundImage: footer.bg_type === 'image' && footer.bg_image ? `url(${footer.bg_image})` : 'none',
    ...
    return (
        ...
        >
            ...
            {footer.bg_type === 'image' && (
                <div className={`absolute inset-0 z-0 ${footer.greyscale ? 'grayscale contrast-125 brightness-50' : 'bg-black/40'}`}>
                    <img src={footer.bg_image} alt="" className="w-full h-full object-cover" />
                </div>
            )}
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-6 py-20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <p className="text-sm leading-relaxed font-light opacity-80" style={{ color: footer.text_color }}>
                            Empresa constructora de Córdoba especializada en obras hidráulicas, viales, saneamiento y ambientales. Profesionales con tecnología de última generación.
                        </p>
                        {/* Redes Sociales */}
                    ...
                    <div>
                        ...
                        <ul className="space-y-3">
                            {['Inicio', 'Servicios', 'Maquinarias', 'Quiénes Somos'].map((item, idx) => {
                                const paths = ['/', '/servicios', '/maquinarias', '/quienes-somos'];
                                return (
                                    <li key={idx}>
                                        <Link to={paths[idx]} className="hover:text-[#fdce27] transition-colors text-sm font-medium opacity-70 hover:opacity-100" style={{ color: footer.text_color }}>
                                            {item}
                                        </Link>
                                    </li>
                                );
                    ...
                    <div>
                        ...
                        <div className="space-y-4">
                            ...
                            {company.address && (
                                <div className="flex items-start gap-3 opacity-70" style={{ color: footer.text_color }}>
                ...
                {(footer.logo1 || footer.logo2) && (
                    <div className="flex flex-wrap justify-center items-center gap-8 mb-12 animate-in fade-in duration-1000">
                        {footer.logo1 && (
                            <img src={footer.logo1} alt="Logo Partner 1" className="h-20 w-auto object-contain transition-all duration-500 rounded-xl bg-white p-2 shadow-2xl shadow-black/20" />
                        )}
                        {footer.logo2 && (
                            <img src={footer.logo2} alt="Logo Partner 2" className="h-20 w-auto object-contain transition-all duration-500 rounded-xl bg-white p-2 shadow-2xl shadow-black/20" />
                        )}
                    </div>
                )}
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-4 text-sm opacity-60" style={{ color: footer.text_color }}>
                    <div className="flex items-center gap-2">
                        <p>© {currentYear} <span className="font-black" style={{ color: footer.text_color }}>Ankaloo Construcciones</span> — Todos los derechos reservados</p>
                    </div>
                    <div className="flex items-center gap-2">
                        <p className="text-sm font-medium">Construyendo el futuro de Córdoba</p>
                    </div>
                </div>
            </div>
